...
<body>
  ...
  <button onclick="liftDrone()">Lift Drone</button>
  <button onclick="descendDrone()">Descend Drone</button>
  <p>Status: <span id="status">Idle</span></p>
  <p>MPU-6050 Readings:</p>
  ...
  <script>
    ...
    function liftDrone() {
      fetch('/lift').then(response => response.text()).then(data => {
        document.getElementById('status').innerText = data;
      });
    }

    function descendDrone() {
      fetch('/descend').then(response => response.text()).then(data => {
        document.getElementById('status').innerText = data;
      });
    }
...
void liftDrone() {
  for (int i = 0; i <= 255; i += 5) {
    motorControl = (i << 1) | (i >> 7); // Slowly increase PWM
    updateShiftRegister();
    delay(50); // Adjust delay for lift speed
  }
  digitalWrite(LED_PIN, HIGH);  // LED on during lift
}

void descendDrone() {
  for (int i = 255; i >= 0; i -= 5) {
    motorControl = (i << 1) | (i >> 7); // Slowly decrease PWM
    updateShiftRegister();
    delay(50); // Adjust delay for descent speed
  }
...
void setup() {
    ...
    server.on("/lift", HTTP_GET, [](AsyncWebServerRequest *request){
      liftDrone();
      request->send(200, "text/plain", "Drone Lifting");
    });

    // Handle descend request
    server.on("/descend", HTTP_GET, [](AsyncWebServerRequest *request){
      descendDrone();
      request->send(200, "text/plain", "Drone Descending");
    });
